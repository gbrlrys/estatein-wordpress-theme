 ...
 *
 * The filter form submits by GET to this same page, so results are
 * bookmarkable and shareable, and it works with JavaScript disabled.
 *
 * @package Estatein
 */
...
// --- Read filters off the query string (all optional). --------------------
$estatein_search   = isset( $_GET['q'] ) ? sanitize_text_field( wp_unslash( $_GET['q'] ) ) : '';           // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Public, read-only filter.
$estatein_type     = isset( $_GET['type'] ) ? sanitize_title( wp_unslash( $_GET['type'] ) ) : '';          // phpcs:ignore WordPress.Security.NonceVerification.Recommended
$estatein_location = isset( $_GET['location'] ) ? sanitize_title( wp_unslash( $_GET['location'] ) ) : '';  // phpcs:ignore WordPress.Security.NonceVerification.Recommended
$estatein_max      = isset( $_GET['max'] ) ? absint( $_GET['max'] ) : 0;                                   // phpcs:ignore WordPress.Security.NonceVerification.Recommended
$estatein_paged    = max( 1, get_query_var( 'paged' ) ? (int) get_query_var( 'paged' ) : ( isset( $_GET['paged'] ) ? absint( $_GET['paged'] ) : 1 ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
...
// With no listings published yet, fall back to the seed set so the page reads
// as designed. Filtering is disabled in that state (there is nothing to filter).
$estatein_items = $estatein_has_real
	? array_map( 'estatein_normalise_property', $estatein_query->posts )
	: estatein_default_properties();
...
<section class="section" aria-labelledby="results-heading">
	<div class="container">
		...
		<!-- Filter bar -->
		<form class="search-bar" method="get" action="<?php echo esc_url( get_permalink() ); ?>" role="search" data-reveal>
			<div class="field">
		...
		<div class="section-head mt-8">
			...
			<?php if ( $estatein_search || $estatein_type || $estatein_location || $estatein_max ) : ?>
				<div class="section-head__action">
					<a class="btn btn--sm" href="<?php echo esc_url( get_permalink() ); ?>">
		...
		<?php else : ?>
			<div class="card text-center" style="padding:56px 24px">
				...
				<p class="lead mt-6"><?php esc_html_e( 'Try widening your price range or clearing a filter — or tell us what you are after and we will search off-market.', 'estatein' ); ?></p>
				<p class="mt-6">
